### Моделирование полёта тела в атмосфере

**Задание:**  
>Реализовать приложение для моделирования полёта тела в атмосфере.  
Предусмотреть возможность ввода шага моделирования и вывода результатов.
Выполнить моделирование без очистки предыдущих результатов для различных шагов моделирования, сравнить траектории и заполнить таблицу:


# **<p style="text-align:center"> Отчёт </p>**

## ***<p style="text-align:center">Метод Эйлера и Рунге-Кута для полёта тела в атмосфере</p>***
### <p style="text-align:center">Математическая модель</p>

$$
\begin{aligned}
\frac{dV_x(t)}{dt} = - k * {V_x(t)} * V(t),\\
\frac{dV_y(t)}{dt} = - g - k * V_y(t) * V(t),\\
V_x(0) = V_0 * cos ɑ, V_y(0) = V_0 * sin ɑ.\\
k = \frac{C * S * \rho}{2m}\\
V(t) = \sqrt{{V_x(t)}^{2} + {V_y(t)}^{2}},\\
m - масса\ тела,\\
C - коэффициент\ лобового\ сопротивления (C ≈ 0.15)\\
S - площадь\ поперечного\ сечения\ тела,\\
\rho - плотность\ воздуха (\rho = 1.29 \frac{кг}{{м}^{3}})
\end{aligned}
$$

**Метод Рунге-Кутта**

$$
\begin{aligned}
\text {Используя выражения выше после апроксимации} \frac{dV(t + ∆t)}{dt} = \frac{V(t + ∆t) - V(t)}{∆t} \text {получаем:}\\
V_x(t + ∆t) = V_x(t) - k * V_x(t) * V(t) * ∆t,\\
V_y(t + ∆t) = V_y(t) - ( g + k * V_y(t) * V(t)) * ∆t,\\
x(t + ∆t) := x(t) + V_x(t + ∆t) * t,\\
y(t + ∆t) := y(t) + V_y(t + ∆t) * t.
\end{aligned}
$$

***
# **<p style="text-align:center">Демонстрация результатов работы программы</p>**
>*Для данных Height = 0, ɑ = 45°, m = 1 кг, v0 = 10 м/с, S = 0.1 м²*

| Шаг моделирования, с | 1 | 0.1 | 0.01 | 0.001 | 0.0001 |
|----------------------|---|-----|------|-------|--------|
| Дальность полёта, м |19.39|10.68|9.58|9.49|9.48|
| Максимальная высота, м |7.07|2.79|2.48|2.45|2.45|
| Скорость в конечной точке, м/с |14.26|9.91|9.34|9.31|9.31|

>Скорость в конечной точке и дальность полёта для таблицы выводил на последней иттерации в файл data.txt.

## **<p style="text-align:center">График</p>**

![График](charts.png)
***
# **<p style="text-align:center">Выводы</p>**

### <p style="text-align: justify;">С уменьшением шага моделирования результаты стремяться к определённому значению:<p>
- *Дальность:* 9.48 м
- *Максимальная высота:* 2.45 м
- *Конечная скорость:* 9.31 м/с

*<p style="text-align: justify;">Это говорит о том, что метод Эйлера **сходится** к какому-то решению.<p>*

- Шаг ∆t = 1 даёт **абсолютно** неверный результаты
- Шаг ∆t = 0.01 и меньше дают **приактически одинаковые** результаты
<p style="text-align: justify;">
Так что для <strong>практических</strong> расчетов можно выбрать шаг dt = 0.01. <strong>Дальнейшее</strong> уменьшение шага (на порядок и более) не имеет смысла.
</p>



